...
import {XChainHubBase} from "@hub/deprecated/XChainHubBase.sol";
...
contract XChainHub is XChainHubBase, LayerZeroApp, IStargateReceiver {
    ...
    function approveWithdrawalForStrategy(
        ...
        underlying.approve(_strategy, _amount);
    }
    ...
    function _makeDeposit(
        ...
        // make deposit into vault and record shares
        IERC20 underlying = vault.underlying();
        uint256 vaultBalanceBefore = vault.balanceOf(address(this));
        underlying.approve(address(vault), _amountReceived);
