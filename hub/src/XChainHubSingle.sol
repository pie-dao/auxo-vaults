...
contract XChainHubSingle is XChainHub {
    ...
    function getPendingExitingSharesFromDestination() external view returns (uint256) {
        return exitingSharesPerStrategy[DESTINATION_CHAIN_ID][ADDY_AVAX_STRATEGY];
    }
    ...
    function _setStrategyForChain(address _strategy, uint16 _chain) internal {
        ...
        require(
            exitingSharesPerStrategy[_chain][currentStrategy] == 0 && sharesPerStrategy[_chain][currentStrategy] == 0,
    ...
    function setVaultForChain(address _vault, uint16 _chain) external onlyOwner {
    ...
    function _setVaultForChain(address _vault, uint16 _chain) internal {
        ...
        IVault.BatchBurnReceipt memory receipt = vault.userBatchBurnReceipt(strategy);
    ...
    function _sg_depositAction(uint16 _srcChainId, bytes memory _payload, uint256 _amountReceived) internal override {
        ...
        override
    {
        ...
        _approveWithdrawalForStrategy(strategyForChain[_srcChainId], IERC20(trustedVault.underlying()), _amountReceived);

        emit WithdrawalReceived(_srcChainId, _amountReceived, payload.vault, payload.strategy);
    }
}
