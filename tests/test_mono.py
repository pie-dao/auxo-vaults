...
def vault(deployer, token):    
    mono_vault = MonoVault.deploy({'from': deployer})
    mono_vault.initialize(token.address, deployer.address, {'from': deployer})
    mono_vault.setLock(False, {'from': deployer})

    return mono_vault

@pytest.fixture
def strategy(deployer, token, vault):
    mock_strategy = MockStrategy.deploy(token.address, vault.address, deployer.address, {'from': deployer})
