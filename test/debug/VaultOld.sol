...
contract Vault is ERC20, Pausable {
    ...
    modifier onlyDepositor(address caller) {
        require(
            auth.isDepositor(IVault(address(this)), caller),
            "error::NOT_DEPOSITOR"
        );
    ...
    modifier onlyAdmin(address caller) {
        require(
            auth.isAdmin(IVault(address(this)), caller),
            "error::NOT_ADMIN"
        );
    ...
    modifier onlyHarvester(address caller) {
        require(
            auth.isHarvester(IVault(address(this)), caller),
            "error::NOT_HARVESTER"
        );
    ...
    function initialize(
        ...
        string memory name_ = string(
            bytes.concat(nPrefix, " ", bytes(underlying_.name()), " ", nSuffix)
        );
        string memory symbol_ = string(
            bytes.concat(sPrefix, bytes(underlying_.symbol()))
        );
    ...
    /// @param newFeePercent The new fee percentage.
    function setBatchedBurningFeePercent(uint256 newFeePercent)
        external
        onlyAdmin(msg.sender)
    {
        // A fee percentage over 100% doesn't make sense.
        ...
        onlyHarvester(msg.sender)
    {
        ...
        if (feesAccrued != 0 && harvestFeeReceiver != address(0)) {
            _mint(
                harvestFeeReceiver,
                feesAccrued.fdiv(exchangeRate(), BASE_UNIT)
            );
        }
    ...
    function pullFromWithdrawalQueue(uint256 underlyingAmount) internal {
        ...
        for (; ; currentIndex--) {
            ...
            uint256 amountToPull = (amountLeftToPull <= strategyBalance)
                ? amountLeftToPull
                : strategyBalance;
            ...
            unchecked {
                ...
                // Withdraw from the strategy and revert if returns an error code.
                require(
                    strategy.withdraw(amountToPull) == 0,
                    "pullFromWithdrawalQueue::REDEEM_FAILED"
                );
            }
    ...
    function computeEstimatedReturns(
        ...
        return
            (invested == 0 || profit == 0)
                ? 0
                : profit.fdiv(invested, BASE_UNIT) *
                    (BLOCKS_PER_YEAR / interval) *
                    100;
    }
}
