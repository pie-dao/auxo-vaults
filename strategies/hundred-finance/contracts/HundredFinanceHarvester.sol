...
import {HundredFinanceStrategy} from "./HundredFinanceStrategy.sol";
import {IHarvester} from "../interfaces/IHarvester.sol";

/// @title HundredFinanceHarvester
/// @author dantop114
/// @notice Harvester contract for HundredFinanceHarvester. This contract can be used to harvest HundredFinanceHarvester.
/// @dev Owner of this contract should set `minRewards` (default 0) and `slippageIn` (default 0)
///      to manage minimum rewards to harvest.
contract HundredFinanceHarvester is IHarvester, Ownable {
    /*///////////////////////////////////////////////////////////////
                                STORAGE
    //////////////////////////////////////////////////////////////*/

    /// @notice The strategy handled by this harvester.
    HundredFinanceStrategy public strategy;
    ...
    function initialize(HundredFinanceStrategy strat, address own) external {
    ...
    function clone(HundredFinanceStrategy strat, address own) external returns (address instance) {
        instance = Clones.clone(address(this));
        HundredFinanceHarvester(instance).initialize(strat, own);
    }
    ...
    function harvest(bytes calldata /* extra */, uint256 deadline) external {
        ...
        HundredFinanceStrategy strategy_ = strategy; // save some SLOADs
        uint256 floatBefore = strategy_.float();

        strategy_.claimRewards();
        strategy_.sellRewards(strategy_.rewardBalance(), minRewards);
